...
| [`35_reconcile_config_demo_diamond.json`](35_reconcile_config_demo_diamond.json) | **Flagship diamond bed.** Two leaves (`orders_src` → `orders_gold_c` / `orders_gold_d`) in `mig_demo_diamond`; shows clustering, blast-radius, distractor isolation, and depth-agnostic trace-back in one story. Deploy [`../demo_diamond/60_demo_diamond_pipeline.sql`](../demo_diamond/60_demo_diamond_pipeline.sql). See [`../demo_diamond/README.md`](../demo_diamond/README.md). |
| [`36_reconcile_config_demo_payments.json`](36_reconcile_config_demo_payments.json) | **Two-tiers bed.** One pair (`payments_src` → `payments_gold`) in `mig_demo_payments`; `payment_ts` resolves **deterministically** (timezone) while `net_amount` (tax-vs-discount business drift) is resolved by the **LLM fallback** via a confirming query. Deploy [`../demo_payments/70_demo_payments_pipeline.sql`](../demo_payments/70_demo_payments_pipeline.sql). See [`../demo_payments/README.md`](../demo_payments/README.md). |
