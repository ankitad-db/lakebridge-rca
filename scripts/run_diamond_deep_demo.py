"""End-to-end driver for the DEEP-DIAMOND bed — the "whole feature set" scenario.

Runs the whole story against a real workspace:

  1. Deploy the diamond pipeline SQL (src leaves + raw_a -> shared_b -> branch_c/d ->
     leaf_e/leaf_f -> report_e/report_f).
  2. App-native reconcile leaf_e_src->leaf_e_tgt AND leaf_f_src->leaf_f_tgt under one recon_id.
  3. Deterministic RCA (probes + drill-down + UC lineage trace-back + code-aware mapping +
     blast radius + clustering).
  4. Tier-2 LLM fallback via a Claude Foundation Model endpoint for the residual (net_amount),
     which promotes the verdict only if its confirming query passes.

Proves in one run: Tier-1 deterministic (amount precision, sku string_format), Tier-2 LLM
(net_amount business drift), deep lineage trace-back to root raw_a, sibling isolation (leaf_e
never surfaces arm D's sku), blast radius (report_* consumers) and clustering.
...
SCHEMA = "mig_demo_diamond_deep"
RECON_SCHEMA = "reconcile"
PIPELINE_SQL = "migration/demo_diamond_deep/72_demo_diamond_deep_pipeline.sql"
MANIFEST = "migration/demo_diamond_deep/table_manifest.yml"

# Claude fallback system prompt (mirrors app/server/llm_fallback.py and run_payments_demo.py).
_SYSTEM = (
...
def reconcile(runner) -> str:
    ...
    print("[recon] reconciling leaf_e_src->leaf_e_tgt AND leaf_f_src->leaf_f_tgt ...")
    res = run_reconcile(
        runner, CATALOG, SCHEMA, SCHEMA,
        specs=[
            TablePairSpec(source_table="leaf_e_src", target_table="leaf_e_tgt", join_keys=["a_id"]),
            TablePairSpec(source_table="leaf_f_src", target_table="leaf_f_tgt", join_keys=["a_id"]),
        ],
...
def llm_fallback(result, runner, mapping, profile: str, endpoint: str) -> int:
    ...
    for f in residuals:
        ...
        tag = f"{f.target_table.split('.')[-1]}.{f.column}"
        print(f"  [{tag}] proposal = {json.dumps(proposal) if proposal else None}")
...
def show_lineage_and_blast(result) -> None:
    """Print the depth-agnostic trace-back path + blast radius evidence per finding, and
    assert sibling isolation (leaf_e must have NO sku finding)."""
    print("\n" + "=" * 82)
    print("LINEAGE TRACE-BACK · BLAST RADIUS · CLUSTERS")
    print("=" * 82)
    for f in result.findings:
        top = f.top_hypothesis
        if not top:
        ...
        for ev in top.evidence:
            if ev.label == "lineage":
                print(f"  [{tag}] {ev.detail}")
    print("\n  clusters:")
    for c in result.clusters:
        cols = ", ".join(c.members)
        print(f"    - {c.signature}: {c.finding_count} finding(s) -> {cols}")

    leaf_e_cols = {f.column for f in result.findings if f.target_table.endswith("leaf_e_tgt")}
    print("\n  SIBLING ISOLATION CHECK (leaf_e must have NO sku):")
    print(f"    leaf_e findings: {sorted(c for c in leaf_e_cols if c)}"
          f"  -> {'PASS (no sku leakage)' if 'sku' not in leaf_e_cols else 'FAIL (sku leaked!)'}")
...
def main(argv=None) -> int:
    ...
    print("[rca] running deterministic analysis (probes + drill-down + lineage + blast + code-aware) ...")
